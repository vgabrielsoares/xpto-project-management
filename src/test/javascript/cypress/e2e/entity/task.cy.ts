...
describe('Task e2e test', () => {
  ...
  const taskSample = { title: 'frightfully', deadline: 2905, status: 'IN_PROGRESS' };
  ...
  describe('new Task page', () => {
    ...
    it('should create an instance of Task', () => {
      cy.get(`[data-cy="title"]`).type('whose terraform');
      cy.get(`[data-cy="title"]`).should('have.value', 'whose terraform');

      cy.get(`[data-cy="description"]`).type('../fake-data/blob/hipster.txt');
      cy.get(`[data-cy="description"]`).invoke('val').should('match', new RegExp('../fake-data/blob/hipster.txt'));

      cy.get(`[data-cy="deadline"]`).type('26960');
      cy.get(`[data-cy="deadline"]`).should('have.value', '26960');

      cy.get(`[data-cy="status"]`).select('ABORTED');
